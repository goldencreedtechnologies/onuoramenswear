  ...
  return (
    <main className="min-h-screen bg-page pt-[104px] text-copy">
      <section className="container-luxe grid gap-12 py-12 md:grid-cols-[0.75fr_1.25fr] md:py-16">
        <div>
          ...
          <div className="mt-8 border-t border-line">
            <a
              ...
              className="gold-focus flex items-center gap-3 border-b border-line py-4 text-sm hover:text-gold"
            >
              <Mail className="h-4 w-4" />
              orders@onuoramenswear.com
            </a>
            <a
              href="mailto:orders@onuoramenswear.com?subject=Styling%20consultation"
              className="gold-focus flex items-center gap-3 border-b border-line py-4 text-sm hover:text-gold"
            >
        ...
        <form
          action="mailto:orders@onuoramenswear.com"
          method="post"
          encType="text/plain"
        ...
        >
          ...
          <label className="grid gap-2 text-[10px] font-semibold uppercase">
            Enquiry
            <select name="enquiry" className={inputClass}>
          ...
          <label className="grid gap-2 text-[10px] font-semibold uppercase">
            Message
            <textarea name="message" required className={`${inputClass} min-h-40 py-4`} />
          </label>
          <button
            type="submit"
            className="gold-focus min-h-12 bg-obsidian px-5 text-xs font-semibold uppercase text-ivory transition hover:bg-gold hover:text-obsidian"
          >
            Send enquiry
          </button>
          <p className="text-xs leading-5 text-copy-muted">
            Sending opens your email application with the enquiry details prepared.
          </p>
        </form>
      </section>
        ...
        <div className="container-luxe grid gap-7 py-10 md:grid-cols-[260px_1fr_auto] md:items-center md:py-12">
          ...
          <div>
            <p className="text-[10px] font-semibold uppercase text-gold">
              {newArrivalsPromotion.title}
            </p>
            <h2 className="mt-2 max-w-2xl text-2xl font-semibold leading-tight">
              {newArrivalsPromotion.offer}
            </h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-copy-muted">
              {newArrivalsPromotion.explanation}
            </p>
          </div>
          <Link
            href={newArrivalsPromotion.href}
            className="gold-focus inline-flex min-h-11 items-center justify-center gap-3 border border-copy px-5 text-xs font-semibold uppercase transition hover:bg-copy hover:text-white"
          >
            Shop arrivals
