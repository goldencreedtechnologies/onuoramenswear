...
function accurateSwatch(name: string, fallback: string, family: string) {
  ...
  if (normalized === "burnt orange") return "#D95F2B";
  return fallback;
}
      ...
      <section className="container-luxe grid gap-6 pb-12 pt-4 md:gap-8 md:pt-8 lg:grid-cols-[minmax(0,1.38fr)_minmax(340px,0.62fr)] lg:gap-12 lg:pb-20">
        ...
        <aside className="lg:sticky lg:top-[124px] lg:self-start">
          <nav className="mb-4 flex flex-wrap items-center gap-1.5 text-[9px] uppercase text-copy-muted" aria-label="Breadcrumb">
            <Link href="/" className="gold-focus hover:text-copy">Home</Link><ChevronRight className="h-3 w-3" />
            <Link href="/collection" className="gold-focus hover:text-copy">Shop</Link><ChevronRight className="h-3 w-3" />
            <span>{collectionLabel}</span><ChevronRight className="h-3 w-3" /><span>{product.colorName}</span><ChevronRight className="h-3 w-3" /><span className="text-copy">Product</span>
          </nav>
          <div className="mb-5 flex items-center justify-between border-y border-line py-2.5">
            <span className="text-[10px] font-semibold uppercase text-copy-muted">Currency</span><CurrencySelector />
          </div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.08em] text-gold">{collection.igboName} · {collectionLabel}</p>
          <h1 className="product-title mt-2 text-3xl font-semibold leading-none sm:text-4xl">{product.colorName}</h1>
          ...
          <div className={styles.options}><ProductOptions product={product} colorOptions={colorOptions} /></div>
          ...
          <details className="group mt-5 border-y border-line py-4">
            <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-semibold uppercase">Read More<span className="text-lg font-light group-open:rotate-45">+</span></summary>
          ...
          <div className="mt-5 grid grid-cols-3 border-b border-line pb-5">
            {[{ icon: PackageCheck, label: "Inventory checked" }, { icon: Globe2, label: "Tracked delivery" }, { icon: ShieldCheck, label: "Secure payment" }].map((item) => {
              const Icon = item.icon;
              return <div key={item.label} className="px-2 text-center"><Icon className="mx-auto h-4 w-4 text-gold" /><p className="mt-2 text-[9px] font-semibold uppercase text-copy-muted">{item.label}</p></div>;
            })}
          </div>
        </aside>
      </section>
