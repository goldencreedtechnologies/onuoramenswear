    ...
    <main className="bg-page pt-[104px] text-copy">
      <div className="container-luxe py-4">
        <nav className="flex items-center gap-1.5 text-[10px] uppercase text-copy-muted" aria-label="Breadcrumb">
          ...
          <Link href={`/collection#${collectionHash}`} className="gold-focus hover:text-copy">{collectionLabel}</Link>
          <ChevronRight className="h-3 w-3" />
      ...
      <section className="container-luxe grid gap-8 pb-14 lg:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)] lg:gap-12 lg:pb-20">
        <ProductGallery images={Array.from(new Set([product.image, ...product.images]))} productName={product.name} />
        <aside className="lg:sticky lg:top-[124px] lg:self-start">
          ...
          <p className="mt-1 text-xs font-semibold uppercase text-copy-muted">{collection.englishName}</p>
          <h1 className="product-title mt-3 text-3xl font-semibold leading-none sm:text-4xl">{product.name}</h1>
            ...
            {[
              ...
              <details key={title} className="group py-4">
                <summary className="flex cursor-pointer list-none items-center justify-between text-xs font-semibold uppercase">
                  {title}<span className="text-lg font-light group-open:rotate-45">+</span>
                </summary>
      ...
      <section className="py-14 md:py-20" style={{ backgroundColor: product.palette, color: product.pageText }}>
        <div className="container-luxe grid gap-6 md:grid-cols-[0.8fr_1.2fr] md:items-start">
          <div>
            ...
            <h2 className="mt-3 max-w-lg text-3xl font-semibold leading-tight md:text-4xl">{product.storyTitle}</h2>
          </div>
          <div>
            <p className="max-w-2xl text-base leading-8" style={{ color: product.pageMuted }}>{product.story}</p>
            <p className="mt-6 text-[10px] font-semibold uppercase" style={{ color: product.pageMuted }}>Designed for {product.occasion}</p>
          </div>
        </div>
      </section>
      ...
      <section className="container-luxe py-14 md:py-20">
        <div className="mb-7 flex items-end justify-between gap-5">
          ...
          <Link href={`/collection#${collectionHash}`} className="gold-focus hidden border-b border-copy/35 pb-1 text-[10px] font-semibold uppercase sm:block">Shop All</Link>
        </div>
