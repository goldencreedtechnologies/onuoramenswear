...
export default function AboutPage() {
  return (
    <main className="bg-page pt-28 text-copy">
      <section className="container-luxe grid gap-12 py-20 md:grid-cols-[0.9fr_1.1fr] md:items-end md:py-28">
        <div>
          <p className="mb-5 text-xs font-bold uppercase tracking-[0] text-gold">Our heritage</p>
          <h1 className="font-display text-6xl leading-[0.9] text-balance md:text-8xl">The voice of the people, cut for now.</h1>
        </div>
        <p className="max-w-xl text-base leading-8 text-copy-muted">
        ...
        <div className="container-luxe">
          ...
          <div className="grid gap-5 md:grid-cols-5">
            {craftSteps.map((step, index) => (
              <div key={step.label} className="rounded-[35px] border border-gold/20 p-6">
                <p className="text-xs font-bold text-gold-soft">0{index + 1}</p>
                <h2 className="mt-8 font-display text-3xl">{step.label}</h2>
                <p className="mt-4 text-sm leading-7 text-ivory/62">{step.text}</p>
              </div>
            ))}
          </div>
        </div>
      </section>
      <section className="container-luxe grid gap-10 py-24 md:grid-cols-2 md:py-32">
        <img src="https://onuoramenswear.com/wp-content/uploads/2025/12/onu-ora.png" alt="Onuora fabric and brand story" className="min-h-[520px] rounded-[35px] object-cover" />
        <div className="flex flex-col justify-center">
          <p className="mb-5 text-xs font-bold uppercase tracking-[0] text-gold">Behind the fabric</p>
          <h2 className="font-display text-5xl leading-[0.95] md:text-7xl">Movement is part of the luxury.</h2>
          <p className="mt-7 text-base leading-8 text-copy-muted">
