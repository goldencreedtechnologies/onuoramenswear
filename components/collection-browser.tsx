  ...
  const content = (
    <>
      ...
      <div className="pt-4">
        ...
        <p className="mt-2 text-[10px] font-medium uppercase tracking-[0.04em] text-copy-muted">{PRODUCT_INCLUSION_LABEL}</p>
      </div>
    </>
  );
    ...
    <div>
      <div className="grid grid-cols-2 gap-2.5 sm:gap-3.5">
        {assets.map((asset, index) => {
          ...
          const image = (
            <div className="relative aspect-[4/5] overflow-hidden bg-[#f1f0ec]">
              <Image src={asset.src} alt={asset.alt} fill priority={priority && index < 2} quality={93} sizes="(min-width: 1024px) 250px, (min-width: 768px) 22vw, 45vw" className={`transition duration-700 ease-out group-hover:scale-[1.015] ${asset.className ?? "object-cover object-top"}`} />
            </div>
          );
  ...
  return (
    <div id="collections" className="overflow-hidden">
      {sections.map((section, sectionIndex) => {
        ...
        return (
          <section key={section.id} id={section.id} aria-labelledby={`${section.id}-title`} className="relative isolate scroll-mt-[112px] overflow-hidden border-b border-line py-10 md:py-14">
            <div className="pointer-events-none absolute inset-0 -z-10" aria-hidden="true">
              <div className="absolute left-1/2 top-[58%] h-[78%] w-[min(98%,1120px)] -translate-x-1/2 -translate-y-1/2 bg-[radial-gradient(circle_at_center,rgba(245,230,200,0.24),rgba(245,230,200,0.09)_40%,transparent_72%)]" />
              <span className={`absolute top-[56%] -translate-y-1/2 select-none font-semibold leading-none ${monogramPosition}`} style={{ fontSize: "clamp(18rem,33vw,34rem)", color: "transparent", WebkitTextStroke: "1px rgba(101,67,33,0.11)" }}>Ọ</span>
            </div>

            <div className="container-luxe relative z-10">
              <header className="mb-8 max-w-3xl md:mb-10">
                <p className="text-xs font-bold uppercase tracking-[0.1em] text-gold">{section.eyebrow}</p>
                <h1 id={`${section.id}-title`} className="mt-3 text-4xl font-bold leading-tight sm:text-5xl md:text-6xl">{section.title}</h1>
                <p className="mt-4 max-w-2xl text-base font-medium leading-7 text-copy-muted md:text-lg md:leading-8">{section.description}</p>
              </header>
              ...
              <div className="mx-auto grid max-w-[1040px] gap-7 md:grid-cols-[minmax(300px,390px)_minmax(420px,520px)] md:items-start md:justify-center md:gap-8 lg:gap-10">
                <div className="mx-auto w-full max-w-[390px] md:max-w-none"><HeroProduct section={section} layout={layout} priority={sectionIndex === 0} /></div>
                <div className="mx-auto w-full max-w-[500px] md:max-w-none"><SupportingGrid section={section} assets={layout.grid} priority={sectionIndex === 0} /></div>
              </div>
            </div>
          </section>
        );
