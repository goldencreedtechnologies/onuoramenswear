...
const fieldClass =
  "gold-focus min-h-12 w-full border border-line bg-page px-4 text-sm font-normal normal-case text-copy outline-none transition placeholder:text-copy-muted/60 hover:border-line-strong focus:border-copy";
...
export function CheckoutClient() {
  ...
  const shipping = deliveryQuote
    ? operationalUsdAmountInCurrency(deliveryQuote.shippingUsd, currency)
    : 0;
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 max-w-md text-sm leading-6 text-copy-muted">
                    {deliveryQuote
                      ? `${deliveryQuote.methodName}: ${deliveryQuote.estimatedMinDays}-${deliveryQuote.estimatedMaxDays} business days${deliveryQuote.distanceKm ? `, approximately ${deliveryQuote.distanceKm} km` : ""}.`
              ...
              items.map((item) => (
                <div key={`${item.productSlug}-${item.size}-${item.colorName}`} className="grid grid-cols-[72px_1fr_auto] gap-3 border-b border-line pb-4">
                  <div className="relative aspect-[4/5] bg-page">
                    <Image src={item.image} alt={item.name} fill sizes="72px" className="object-cover object-top" />
                  </div>
